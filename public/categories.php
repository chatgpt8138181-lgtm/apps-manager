...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($action === 'add') {
            ...
            redirect_with('categories.php', 'success', 'Console name added.');
        }
        if ($action === 'delete') {
            delete_category((int) ($_POST['category_id'] ?? 0));
            redirect_with('categories.php', 'success', 'Console name deleted.');
        }
    } catch (Throwable $e) {
        ...
        <table>
            ...
            <?php foreach ($categories as $category): ?>
                <tr>
                    ...
                    <td>
                        <form method="post" onsubmit="return confirm('Delete this console name and its apps?');">
                            <?= csrf_field() ?>
